...
const DEFAULT_PATTERNS: Record<string, RegExp[]> = {
  'food-grocery': [
    ...
    /cocci\s*market|proxy|8\s*a\s*huit|vival|spar|netto/i,
    /primeur|marche|epicerie|boucherie|boulangerie/i,
  ],
  'food-restaurant': [
    /mc\s*donald|mcdonald|burger\s*king|kfc|quick|five\s*guys/i,
    /pizza|sushi|restaurant|brasserie|traiteur/i,
    /uber\s*eats|deliveroo|just\s*eat|frichti/i,
    /cafe|bistro|bar|snack|kebab|thai|chinois|japonais|indien/i,
    /subway|domino|papa\s*john|buffalo|hippo|courtepaille|flunch/i,
    /starbucks|columbus|bagel|pret\s*a\s*manger/i,
  ],
  transport: [
  ...
  shopping: [
    /fnac|darty|boulanger|cdiscount|electro\s*depot/i,
    /zalando|asos|vinted|leboncoin|vestiaire|shein/i,
    /zara|h&m|uniqlo|decathlon|go\s*sport|intersport/i,
    /bershka|pull\s*(&|and)\s*bear|stradivarius|massimo\s*dutti/i,
    /mango|primark|kiabi|c&a|celio|jules|cache\s*cache/i,
    /nike|adidas|puma|new\s*balance|foot\s*locker|courir/i,
